...
class move_bot:
    ...
    def control(self,h,t):
       ...
       #FOV constraints
        ...
        m_lidar = alpha_m*(X/Z)+m0 
        n_lidar = alpha_n*(Y/Z)+n0 

        m_cam = self.marker_center[0]
        n_cam = self.marker_center[1]

        #Check if the camera can see the marker
        #If yes, fuse measurements, if no, let lidar take over
       
        m = 0.5*m_cam+0.5*m_lidar 
        n = 0.5*n_cam+0.5*n_lidar
