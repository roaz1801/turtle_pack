...
if __name__ == '__main__':
    ...
    for i in range(len(exp1_pos[3][:3000])):
        ...
        exp4_pos[3][i] = math.degrees(math.atan2(np.sin(exp4_pos[1][i]-exp4_pos[2][i]),np.cos(exp4_pos[1][i]-exp4_pos[2][i])))
    ...
    plt.plot(exp1_pos[0][:3000],exp1_pos[3][:3000],label="Camera-only",color = "blue")
    plt.plot(exp2_pos[0][:3000],exp2_pos[3][:3000],label="Lidar-only",color = "green")
    plt.plot(exp3_pos[0][:3000],exp3_pos[3][:3000],label="Filtered",color = "red")
    plt.plot(exp4_pos[0][:3000],exp4_pos[3][:3000],label="Complete",color = "orange")
